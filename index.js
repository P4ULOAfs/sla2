...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN: BRUXINHO MODS\n'
            + 'ORG:Owner BRUXINHO;\n'
            + 'TEL;type=CELL;type=VOICE;waid=19034375199:+1 903 437-5199\n'
            + 'END:VCARD'
...
limitt = 10
ban = []
userpremium = ["19034375199@s.whatsapp.net"] //ubah nomer kalian
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('você já mamou o Paulo ')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```Lista Banido``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista de Premium do Paulo:)``` :\n'
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('19034375199@s.whatsapp.net', options, text, {quoted: mek})
